...
- サーバの構築 … [01-setup.md](./01-setup.md)
- 画面の操作 … [02-webui.md](./02-webui.md)
- つまずいた点 … [99-troubleshooting.md](./99-troubleshooting.md)

設定した監視の種類は以下を参照。

- [HTTP 監視（Web シナリオ）](./04-http.md)
- [Docker 監視](./05-docker.md)
- [SSL 証明書の監視](./06-cert.md)
...
監視対象側で実行する。Amazon Linux 2023 でも **エージェントは導入できる**（サーバは導入できない。理由は [99-troubleshooting.md](./99-troubleshooting.md) を参照）。
...
本環境では、`Linux by Zabbix agent` を1つ適用しただけで **50項目**が生成された。その後ローレベルディスカバリが動き、ファイルシステムとネットワークインターフェースを検出して **77項目**まで増加した。LLD の詳細は [05-docker.md](./05-docker.md) を参照。
