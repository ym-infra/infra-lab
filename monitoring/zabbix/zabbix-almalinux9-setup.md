...
バージョンは 7.0 LTS を選定した。7.4 は標準リリースであり次期 LTS が出るまでの短期サポートとなるため、実務での提案を想定して LTS を採用している。

**OS は AlmaLinux 9 を用いる。Amazon Linux 2023 では構築できなかった。**
その理由と経緯は [トラブルシューティング](./troubleshooting.md) に記載する。
...
複数人で運用する場合や、社内標準で root パスワードの設定が求められる場合はこちらを用いる。初期セキュリティ設定をまとめて実施できる `mysql_secure_installation` を使う。
